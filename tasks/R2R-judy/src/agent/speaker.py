...
This implementation is based on https://github.com/airsplay/R2R-EnvDrop
...
class Speaker(object):
    ...
    def __init__(self, 
        ...
        device:torch.device,
        tok:utils.Tokenizer,
        env:environ.R2RBatch=None,
        img_feature_size:int=2048,
        angle_feat_size:int=128, 
    ):
        self.device = device
        self.env = env
        self.img_feature_size = img_feature_size
        if self.env is not None: assert self.img_feature_size == self.env.feature_size
        ...
        self.encoder = M.SpeakerEncoder(
            self.img_feature_size+self.angle_feat_size, 
            self.cfg.RNN_DIM, 
            self.cfg.DROPOUT, 
            self.cfg.BI_DIRECTION,
            self.angle_feat_size,
            self.cfg.FEAT_DROPOUT)
        self.decoder = M.SpeakerDecoder(
            self.tok.vocab_size(), 
            self.cfg.WEMB, 
        ...
        self.encoder_optimizer = torch.optim.Adam(self.encoder.parameters(), lr=self.cfg.LR)
        self.decoder_optimizer = torch.optim.Adam(self.decoder.parameters(), lr=self.cfg.LR)
    ...
    def valid(self, *aargs, **kwargs):
        ...
        self.env.reset_epoch(shuffle=True)
        N = 1 if self.cfg.FAST_TRAIN else 3     # Set the iter to 1 if the fast_train (o.w. the problem occurs)
        metrics = np.zeros(3)
        for i in range(N):
            self.env.reset()
    ...
    def _candidate_variable(self, obs, actions):
        candidate_feat = np.zeros((len(obs), self.img_feature_size + self.angle_feat_size), dtype=np.float32)
    ...
    def from_shortest_path(self, viewpoints=None, get_first_feat=False):
        ...
        img_feats = []
        can_feats = []
        first_feat = np.zeros((len(obs), self.img_feature_size+self.angle_feat_size), np.float32)
        for i, ob in enumerate(obs):
            first_feat[i, -self.angle_feat_size:] = utils.ImageFeatures.make_angle_feat(ob['heading'], ob['elevation'])
    ...
    def teacher_forcing(self, train=True, features=None, insts=None, for_listener=False):
        ...
        if features is not None:
            ...
            ctx = self.encoder(can_feats, img_feats, lengths)
        h_t = torch.zeros(1, batch_size, self.cfg.RNN_DIM).cuda()
        c_t = torch.zeros(1, batch_size, self.cfg.RNN_DIM).cuda()
        ctx_mask = utils.length2mask(lengths, self.device)
    ...
    def infer_batch(self, sampling=False, train=False, featdropmask=None):
        ...
        if featdropmask is not None:
            img_feats[..., :-self.angle_feat_size] *= featdropmask
            can_feats[..., :-self.angle_feat_size] *= featdropmask
        ...
        for i in range(self.cfg.MAX_DECODE):
            # Decode Step
    ...
    def load(self, path, cuda:int=0):
        ''' Loads parameters (but not training state) '''
        print("Load the speaker's state dict from %s" % path)
        states = torch.load(path, map_location=f"cuda:{cuda}")
    ...
    def _feature_variable(self, obs, num_views:int=36):
        ...
        feature_size = obs[0]['feature'].shape[-1]
        _shape = (len(obs), num_views, feature_size)
        features = np.empty(_shape, dtype=np.float32)
        for i, ob in enumerate(obs):
            features[i, :, :] = ob['feature']   # Image feat with angle info, 2048 + 128
